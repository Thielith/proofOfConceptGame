...
class player:
    def __init__(self, name, accuracy, speed, toughness, score):
        self.name = name
...
def fight(self, enemy):
    ...
    def calcScore(self, enemy):
        ...
        updatePlayerFromDB(cur, self.name, self.accuracy, self.speed, self.toughness, self.score)
        print("commit update")
        db.commit();
        print("close")
        db.close();
...
p = player(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])
e = player(sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10])
